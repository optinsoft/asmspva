...
async def testAsyncSmsPva(apiKey: str):
    ...
    await testApi('getCountNew:', asmspva.getCountNew('opt4','RU'))
    await testApi('getServicePrice', asmspva.getServicePrice('opt4','RU'))
    number = await testApi('getNumber', asmspva.getNumber('opt4','RU'))
    if number:
        print(number)    
        await testApi('getSMS', asmspva.getSMS('opt4','RU',number['id']))
        ...
        await testApi('denial', asmspva.denial('opt4','RU',number['id']))
